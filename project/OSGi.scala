...
object OSGi {
  ...
  val httpCore = exports(Seq("akka.http.*"), imports = Seq())
  ...
  def defaultImports(scalaVersion: String) = Seq("!sun.misc", akkaImport(), configImport(), "!scala.util.parsing.*", scalaImport(scalaVersion), "*")
